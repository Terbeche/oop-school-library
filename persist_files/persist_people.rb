...
module PersistPeople
  def store_people(persons)
    ...
    persons.each do |person|
      case person
      when Student
        persons_array << { age: person.age, name: person.name, parent_permission: person.parent_permission }

      when Teacher
        persons_array << { age: person.age, name: person.name, parent_permission: 'hola',
                           specialization: 'math' }
